...
fall_params = {
    'max_zero_allowed_per_year': 364,
    'max_nan_allowed_per_year': 364,
    'min_flow_rate': 0.001, # Don't calculate flow metrics if max flow is befow this value.
    'sigma': 0.05,  # Smaller filter to find fall flush peak
    'broad_sigma': 15,  # Larger filter to find wet season peak
    'wet_season_sigma': 12,  # Medium sigma to find wet season initation peak
    'peak_sensitivity': 0.05, #0.05  # smaller value detects more peaks ##0.1 used, 0.05used for canyon
    'peak_sensitivity_wet': 0.005, # larger value used for detection of wet season initiation
    'max_flush_duration': 40,  # Maximum duration from start to end, for fall flush peak
    'min_flush_percentage': 0.01, # minimum flush, to satisfy the min required to be called a flush
    ...
    'flush_threshold_perc': 0.3, # Size of flush peak, from rising limb to top of peak, has great enough change
    'min_flush_threshold': 0.001, # minimum allowable magnitude threshold for fall flush flow
    'date_cutoff': 366, # Latest accepted date for fall flush, in Julian Date counting from Oct 1st = 0. (i.e. Dec 15th = 75, 366 = Sep30th)
