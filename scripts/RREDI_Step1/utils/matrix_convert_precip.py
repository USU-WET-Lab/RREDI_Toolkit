...
class MatrixConversionP:

    def __init__(self, date_array, precip_array,  start_date):
        self.precip_array = precip_array
        ...
        self.precip_matrix = None
        self.get_date_arrays()
        self.get_precip_matrix()
    ...
    def get_precip_matrix(self):

        number_of_columns = len(self.year_array)

        precip_matrix = np.zeros((366, number_of_columns))
        precip_matrix.fill(None)
        ...
        for index, julian_date in enumerate(self.julian_array):
            ...
            precip_matrix[row][column] = self.precip_array[index]

        self.precip_matrix = precip_matrix
...
def convert_raw_data_to_matrix(fixed_df, current_gauge_column_index, start_date):
    ...
    precip_matrix = get_precip_matrix(
        years, julian_dates, flow_column, year_ranges, start_date)
    return current_gauge_class, current_gauge_number, year_ranges, precip_matrix, julian_dates
...
def get_precip_matrix(years, julian_dates, flow, year_ranges, start_date):
    ...
    for index, julian_date in enumerate(julian_dates):
        ...
        precip_matrix[row][column] = flow[index]
